...
class Model:
    ...
    def __init__(self, agent_count: int = 999, *, learning_rate: float = 0.3, confidence_threshold: float = 0.5):
        ...
        self.ticks = []
        self.advocates_count = []  # opinion >= 0.5 かつ aware なエージェント数
    ...
    def _record(self):
        ...
        advocates = [t for t in know_agents if t.expressed_opinion is not None and t.expressed_opinion > 0.5]
        ...
        self.advocates_count.append(len(advocates))
...
def run_multiple_simulations_advocates(n_runs: int = 100, ticks: int = 250,
                                     *, agent_count: int = 999, learning_rate: float = 0.3,
                                     confidence_threshold: float = 0.5):
    """advocates (opinion > 0.5 かつ aware) の数をグラフ化"""
    all_advocates = []
    for _ in range(n_runs):
        ...
        all_advocates.append(model.advocates_count)

    all_advocates = np.array(all_advocates)
    mean = np.mean(all_advocates, axis=0)
    std = np.std(all_advocates, axis=0)
    ci = 1.96 * std / np.sqrt(n_runs)

    t = np.arange(ticks + 1)
    plt.figure(figsize=(3.5, 4))
    plt.plot(t, mean, color='green', linewidth=2, label=f'Average over {n_runs} runs')
    plt.fill_between(t, mean - ci, mean + ci, alpha=0.1, color='green', label='95% CI')
    plt.xlabel('Time', fontsize=12)
    plt.ylabel('Number of Advocates\n(Opinion ≥ 0.5)', fontsize=12)
    plt.text(-0.2, 1.1, '(b)', transform=plt.gca().transAxes,
         fontsize=15, fontweight='bold', va='top', ha='left')
    plt.xticks(np.arange(0, ticks+1, 50))  
    plt.yticks(np.arange(0, max(mean)+10, 20)) 
...
def create_panel_figure(n_runs=5, base_ticks=600):
    ...
    for i, lr in enumerate(learning_rates):
        for j, ct in enumerate(confidence_thresholds):
            ...
            all_advocates = []  # sum_opinionsからadvocatesに変更
            for run in range(n_runs):
                ...
                model.run(ticks=ticks)
                all_advocates.append(model.advocates_count)  # advocates_countを使用
            
            # Calculate statistics
            all_advocates = np.array(all_advocates)  # all_sumsからall_advocatesに変更
            mean = np.mean(all_advocates, axis=0)
            std = np.std(all_advocates, axis=0)
            ci = 1.96 * std / np.sqrt(n_runs)
    ...
    for i, lr in enumerate(learning_rates):
        for j, ct in enumerate(confidence_thresholds):
            ...
            t = np.arange(ticks + 1)
            ax.plot(t, mean, color='green', linewidth=1.5)  # 緑色に変更
            ax.fill_between(t, mean - ci, mean + ci, alpha=0.2, color='green')  # 緑色に変更
            ...
            if j == 0:
                ax.set_ylabel('Number of Advocates', fontsize=14, fontweight='medium')  # Y軸ラベルを変更
            else:
    ...
    plt.savefig('advocates_comparison_grid_variable_ticks.png', dpi=600, bbox_inches='tight')  # ファイル名変更
    plt.savefig('advocates_comparison_grid_variable_ticks.svg', dpi=600, bbox_inches='tight')  # SVG形式でも保存
    plt.show()
