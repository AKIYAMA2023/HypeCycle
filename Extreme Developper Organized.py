...
class Turtle:
    ...
    def __init__(self, x: float | None = None, y: float | None = None, latent_opinion: float = None):
...
class Model:
    ...
    def setup(self):
        ...
        developer = Turtle(latent_opinion=1.5)
        developer.expressed_opinion = 1.5
        developer.know = 1
    ...
    def plot_advocate_count_trend(self):
        ...
        # 軸の設定
        plt.xticks(np.arange(0, len(self.ticks), 100))
    ...
    def plot_combined_trends(self):
        ...
        # 軸の設定
        ax1.set_xticks(np.arange(0, len(self.ticks), 100))
        ax2.set_xticks(np.arange(0, len(self.ticks), 100))
        ax2.set_yticks(np.arange(0, 101, 20))
        
        plt.tight_layout()
        plt.savefig('High_combined_opinion_sum_and_awareness_rates.png', dpi=300, bbox_inches='tight')
        plt.savefig('High_combined_opinion_sum_and_awareness_rates.svg', dpi=300, bbox_inches='tight')
        plt.show()

    def plot_advocate_count(self):
        """Plot the number of advocates (aware agents with expressed_opinion > 0.5) over time."""
        advocate_counts = [sum(1 for t in self.turtles if t.know == 1 and t.expressed_opinion is not None and t.expressed_opinion > 0.5) for _ in self.ticks]

        plt.figure(figsize=(6, 4))
        plt.plot(self.ticks, advocate_counts, 'g-', alpha=0.7, linewidth=2, label='Advocate Count')

        plt.xlabel('Time Step (t)', fontsize=12)
        plt.ylabel('Number of Advocates', fontsize=12)
        plt.xlim(left=0, right=len(self.ticks))
        plt.ylim(bottom=0, top=max(advocate_counts) * 1.1 if advocate_counts else 10)
        plt.grid(True, linestyle='-', linewidth=0.5, color='gray', alpha=0.3)
