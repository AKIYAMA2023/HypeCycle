    ...
    def plot_opinion_evolution_clusters_1d_horizontal(self, time_points=[15, 25, 40, 75, 120]):
        ...
        for idx, tick in enumerate(time_points):
            ...
            if len(aware_indices) <= 1:
                ax.text(0, 0.5, f"Time {tick}\nNot enough aware agents",
                       ha='left', va='center', transform=ax.transAxes, fontsize=14, fontweight='bold')
                continue
            ...
            panel_label = panel_labels[idx] if idx < len(panel_labels) else f'({chr(97+idx)})'
            ax.set_title(f'{panel_label} t = {tick}', fontsize=20, fontweight='bold', loc='left')
            ax.grid(True, alpha=0.2)
            ax.set_xlim(-1.6, 1.6)
            ax.set_ylim(-1.6, 1.6)
           
            # X軸ラベル（時刻ごとの意見）
            ax.set_xlabel(f'Expressed Opinion', fontsize=20)
           
            # Y軸ラベル（初期意見）は最初のサブプロットにのみ表示
            if idx == 0:
                ax.set_ylabel('Latent Opinion', fontsize=20)
           
            # 認知者の相関係数を計算
            if len(aware_indices) > 1:
                correlation = np.corrcoef(current_opinions, initial_opinions)[0, 1]
            else:
                correlation = np.nan
            
            # クラスター数、認知率、相関係数を表示
            ax.text(0.05, 0.95, f'Aware: {awareness_rate:.1f}%\nr = {correlation:.2f}',
